...
export const getUnverified = (org, chapter) => {
    return async (dispatch) => {
        try {
            await getDocs(collection(getFirestore(),"organizations/" + org + "/chapters/" + chapter + "/unverified"))
            .then((docs) => {
                const list = [];
                docs.forEach((doc) => {
                    list.push({id: doc.id, data: doc.data()})
                });
                dispatch({ type: UNVERIFIED_LIST_LOAD, unverified: list });
            })
        } catch (error) {
            console.log(error);
        }
                ...
                if (docSnap.exists()) {
                    ...
                    .then(() => {
                        ...
                        .then(() => {
                            ...
                            dispatch({ type: VERIFY, unverified: list });
                        })
                    })
                }
            })
